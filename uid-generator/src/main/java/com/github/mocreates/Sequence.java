...
public class Sequence {
    ...
    public Sequence(SequenceConfig config) {
        ...
        long maxWorkerId = ~(-1L << config.getWorkerIdBits());
        long maxDatacenterId = ~(-1L << config.getDatacenterIdBits());
        ...
        this.sequenceMask = ~(-1L << config.getSequenceBits());

        if (config instanceof DefaultSequenceConfig) {

            DefaultSequenceConfig defaultSequenceConfig = (DefaultSequenceConfig) config;
            Assert.isTrue(defaultSequenceConfig.getWorkerId() <= maxWorkerId && defaultSequenceConfig.getWorkerId() >= 0,
                    String.format("worker Id can't be greater than %d or less than 0", maxWorkerId));
            Assert.isTrue(defaultSequenceConfig.getDatacenterId() <= maxDatacenterId && defaultSequenceConfig.getDatacenterId() >= 0,
